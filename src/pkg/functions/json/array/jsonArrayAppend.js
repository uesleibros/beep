...
async function updateValue(obj, path, value, errorRef, message) {
	...
	if (!obj.hasOwnProperty(key)) {
		await message.channel.send(`\`$jsonArrayAppend\` unable to traverse the path: **${key}** does not exist.`);
	...
	if (path.length === 0) {
		if (!Array.isArray(obj[key])) {
			await message.channel.send(`\`$jsonArrayAppend\` unable to traverse the path "**${key}**", it's not an array.`);
			errorRef.current = true;
			return;
		}
...
async function jsonArrayAppend(code, client, message, raw, options) {
	...
	let error = await FunctionError("jsonArrayAppend", ["string:unlimited", "string:non-op"], args, false, options.originalCode, raw, message);

	if (!error) {
		if (!options.json.object) {
			await message.channel.send("`$jsonArrayAppend` unable to work, try define a json using: `$jsonParse[...]`.");
			error = true;
		} else {
